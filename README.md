...
This tool mainly targets software developers and testers that should evaluate the quality of software of cloud-based applications
...
ITER allows search to be bootstrapped by providing a test report as input. If this happens, ITER do not repeat the execution, but it evaluates from scratch all the provided assertions. This is useful for example when new assertions are available, and the test report must be updated to account for them.
...
ITER can be configured also to repeat previously run test executions, indeed implementing a for of regression test.
