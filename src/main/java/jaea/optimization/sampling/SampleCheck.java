...
import jaea.optimization.tools.Utils;
...
public class SampleCheck {
	 ...
	 * 
	 * @param samplePoints
	 *            Matrix of sample points: row = number of samples, col =
	 *            dimensions
	 * @param lowerBound
	 *            Lower bounds on each dimension
	 * @param upperBound
	 *            Upper bounds on each dimension
	 * @param nBin
	...
	static public boolean isStratifiedSample(double[][] samplePoints,
			double[] lowerBound, double[] upperBound, int nBin) {
		boolean res = true;
		...
		try {
			for (int i = 0; i < nSample; i++) {
				...
				for (int j = 0; j < nDim; j++) {
					indexDim = (int) Math.floor((chrom[j] - lowerBound[j])
							/ binWidth[j]);
					if (indexDim == nBin)
						indexDim = nBin - 1;
			...
			System.err.println("Error at Stratified Sample checking... "
					+ e.getMessage());
			System.exit(0);
		}
	...
	public static void main(String[] args) {
		...
		double[][] samples = Utils.File2DoubleMat("p112.csv");
		...
		/*
		 * for (int j = 0; j < nDim; j++) { highB[j] += 0.01*Math.abs(highB[j]);
		 * }
		 */
